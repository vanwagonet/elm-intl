...
all =
  let
    fromLocaleTests = describe "fromLocale"
      [ test "formats numbers" <|
        \() ->
          let
            format = NumberFormat.format (NumberFormat.fromLocale Locale.en)
          in
            Expect.equal (format 12345.6789) "12,345.679"
      ]
    fromOptionsTests = describe "specified options"
      [ let
          options =
            { defaults
          ...
          format = NumberFormat.format (NumberFormat.fromOptions options)
          resolved = NumberFormat.resolvedOptions (NumberFormat.fromOptions options)
        in
          describe "percent"
            [ test "format" <|
