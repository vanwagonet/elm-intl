...
all =
  let
    fromLocaleTests = describe "fromLocale and format"
      [ test "English" <|
        \() ->
          let
            format = DateTimeFormat.format (DateTimeFormat.fromLocale Locale.en)
          in
            Expect.equal
    ...
    fromOptionsTests = describe "fromOptions and resolvedOptions"
      ( map
        (\options ->
          ...
          \() ->
            let
              resolved = DateTimeFormat.resolvedOptions (DateTimeFormat.fromOptions options)
            in
              Expect.true "Expected resolved options to not have something match"
                ( resolved.weekday == options.weekday
                || resolved.era == options.era
                || resolved.year == options.year
                || resolved.month == options.month
                || resolved.day == options.day
                || resolved.hour == options.hour
                || resolved.minute == options.minute
                || resolved.second == options.second
                || resolved.timeZoneName == options.timeZoneName
                )
        )
        optionCombinations
      )
    ...
    defaultsTests = describe "defaults"
      [ test "end up being m/d/y" <|
        \() ->
          ...
          in
            Expect.true "mdy"
              ( resolved.year == DateTimeFormat.NumericNumber
              && resolved.month == DateTimeFormat.NumericMonth
              && resolved.day == DateTimeFormat.NumericNumber
              )
      ]
  in
